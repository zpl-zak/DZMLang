...
tailcall:
     ...
     else if(is_tagged_list(Exp, ArrayAccessSymbol))
     {
          Exp = pair_get_b(Exp);
          OBJECT *Array = lookup_variable_value(pair_get_a(Exp), Env);
          return(pair_nth(Array, pair_get_a(pair_get_b(Exp))->uData.MDL_FIXNUM.Value));
     }
