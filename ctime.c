...
// This was written for Linux and OS X
            ...
            else if(strcmp(Mode, "-end") == 0)
            {
                   ...
                   (fread(&LastEntry, sizeof(LastEntry), 1, Handle) == 1))
                {
                    if(!(LastEntry.Flags & TFEF_Complete))
                    {
                        ...
                        {
                            // NOTE: Assume timer wrap
                            LastEntry.MillisecondsElapsed = (StartClockD - EndClockD);
                        }
